...
import pandas as pd
import threading
from save_bank_data.config import conn, bank_accounts
...
class BankDetail():
    ...
    def updata_balance(self, account, tdate, ydate):
        ...
        sql = """
               ...
               INNER JOIN RDS_CAS_SRC_Iexpenditure b on a.FAccount=b.FAccount and a.[FDate] = b.[FDate]  where a.account = '{}'
               and a.[date] = '{}' 
               """.format(account, tdate)
        df = pd.read_sql(sql, self.engine)
        new_sql = """select balance from RDS_CAS_SRC_Details where date = '{}' and account = '{}'
        """.format(ydate, account)
        update_df = pd.read_sql(new_sql, self.engine)
        today_sql = """select balance from RDS_CAS_SRC_Details where date = '{}' and account = '{}'
        ...
        if not today_df.empty:
            ...
            up_sql = """update a set a.FCode='{}',a.FBalance='{}',a.FIncome='{}',a.expenditure='{}' from RDS_CAS_SRC_Details a where  FDate = '{}' and FAccount = '{}'
